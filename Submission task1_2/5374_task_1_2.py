		...
		#Sets the PID
                ...
                if(self.currenttime-self.lasttime>=self.sample_time):
                	# If the sample time has elapsed, tune PID
		                ...
		                self.cmd.rcPitch=1500+self.error[0]*self.Kp[0]+self.dif_error[0]*self.Kd[0]/self.sample_time+self.sum_values[0]*self.Ki[0]*self.sample_time
		                self.cmd.rcRoll=1500+self.error[1]*self.Kp[1]+self.dif_error[1]*self.Kd[1]/self.sample_time+self.sum_values[1]*self.Ki[1]*self.sample_time
		                self.cmd.rcThrottle=1500+self.error[2]*self.Kp[2]+self.dif_error[2]*self.Kd[2]/self.sample_time+self.sum_values[2]*self.Ki[2]*self.sample_time
		                self.cmd.rcYaw=1500+self.error[3]*self.Kp[3]+self.dif_error[3]*self.Kd[3]/self.sample_time+self.sum_values[3]*self.Ki[3]*self.sample_time
		...
		if e_drone.success == 1:
			...
			if e_drone.initial != 0:
				...
				if e_drone.counter==e_drone.pointsInPath:
					...
					if errX<0.5 and errY<0.5 and errZ<0.5:
						print("Disarm")
						e_drone.disarm()
						break
