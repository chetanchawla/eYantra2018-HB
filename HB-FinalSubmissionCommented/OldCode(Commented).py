...
class Edrone():
	...
	* Example Call: Called when an Edrone object is made
	...
	def __init__(self):
		...
		#new variables ........ by Divyansh###################################################################################################
		# # indicating Lua which tree or node to find the path to 
		# red =1
		# yellow= 2
		# green =3
		# init =4
		...
		self.initial_waypoint = [0.0,0.0,0.0,0.0] # whycon marker at the position of the initial waypoint in the scene (blue sphere). These are whycon coordinates of the same
		...
		#w, h = 4, 41  -- commented by divyansh
		w=4
		self.path = [[0.0 for x in range(w)] for y in range(self.h)] 
			...
			#Check for the error to be less than the maximum acceptable error in x, y, z from current position and setpoint
				for i in range(0,4):
					if(i==0 or i==1):
						...
						if(self.counter == self.h):# If all the points in the path are covered
							self.success=1
...
if __name__=="__main__":
	...
	for i in range(0,e_drone.pno):
		...
		print("path received for goal" + str(i))
		e_drone.pathreceived = 0
	#print("half path received")
	# y=e_drone.pointsInPath-1
	# for i in range(e_drone.pointsInPath+1,41): #set up the reverse path to complete the loop using the previous path points itself, hence decreasing processing required
