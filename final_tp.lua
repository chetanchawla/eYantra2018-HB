...
function sysCall_init()
    ...
    goal1_handle=sim.getObjectHandle('Front_hoop3')
    goal1_back=sim.getObjectHandle('Back_hoop3')
    ...
    no_of_path_points_required=19-- Add no of path points you want from one point to another
...
function packdata(path)
    ...
    if inoutofloop == 0 then
        ...
        -- posef=getpose(goal1_handle,-1)
        -- a = {x = posef[4], y = posef[5], w = posef[6], z = posef[7]}
        -- b = {x = posef[1], y = posef[2], z = posef[3]}
        -- pose = {position = b, orientation = a, }
...
function key_callback(msg)
    ...
    elseif msg.data==4 then
        ...
        sim.setObjectPosition(cashew,-1,{wx,wy-0.50,wz})
