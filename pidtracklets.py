...
class Edrone():
	...
	def __init__(self):
		...
		self.drone_init=[0.0,0.0,0.0,0.0]
	...
	def whycon_callback(self,msg):
		...
		#--------------------Set the remaining co-ordinates of the drone from msg----------------------------------------------
                ...
                # if self.init==1:
                # 	self.drone_init=self.drone_position
                # 	self.init=0
		#---------------------------------------------------------------------------------------------------------------
	...
	#	8. Add error_sum
                ...
                if(iter<10):
	                if ((abs(self.error[0])<=errorthresh) and (abs(self.error[1])<=errorthresh) and (abs(self.error[2])<=errorthresh) and (abs(self.error[3])<=errorthresh)):
	                	self.subsuccess=1
	                	return
	...
	def findpoints(self):
		self.drone_init=self.drone_position
		dif=self.setpoint-self.drone_init
		for i in range(0,10):
			temp=[self.drone_init[0]+(i+1)*dif[0]/10,self.drone_init[1]+(i+1)*dif[1]/10,self.drone_init[2]+(i+1)*dif[2]/10,self.drone_init[3]+(i+1)*dif[3]/10,]
			self.subsetpoints[i]=temp
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		if once=1:
			for i in range(0,10):
				e_drone.setpoint=e_drone.subsetpoints[i]
				while(e_drone.subsuccess!=1):
					e_drone.pid(i)
				e_drone.sum_values=[0.0,0.0,0.0,0.0]
				e_drone.iterations=1
				e_drone.subsuccess=0
			once=2
		e_drone.pid(10)
